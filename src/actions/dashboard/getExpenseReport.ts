...
import { IExpenseReportsResponse } from '@/types/dashboard';
...
export const getExpenseReport = async (
  ...
  try {
    ...
    const res = await authApi.get(`/overview/expense?${params}`);
    ...
    if (res.ok) {
      return {
        ...
        data: data as ICommonGetResponse<IExpenseReportsResponse>,
      };
    }
    if (!res.ok) {

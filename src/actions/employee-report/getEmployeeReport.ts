...
import { IEmployeeReportsResponse } from '@/types/Employee';
...
export const getEmployeeReport = async (
  ...
  try {
    ...
    const res = await authApi.get(`/overview/employee?${params}`);
    ...
    if (res.ok) {
      return {
        ...
        data: data as ICommonGetResponse<IEmployeeReportsResponse>,
      };
    }
    if (!res.ok) {

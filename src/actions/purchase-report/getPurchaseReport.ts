...
import { IPurchaseReportResponse } from '@/types/PurchaseReport';
...
export const getPurchaseReport = async (
  ...
  try {
    ...
    const res = await authApi.get(`/overview/purchase?${params}`);
    ...
    if (res.ok) {
      return {
        ...
        data: data as ICommonGetResponse<IPurchaseReportResponse>,
      };
    }
    if (!res.ok) {

...
export const RightSection = ({ data }: { data: IUserResponse | undefined }) => {
            ...
            <div className="px-space16 my-space8 border-b border-primary-20 dark:border-primary-80">
                <div className="flex gap-space16 items-center justify-between py-space8">
                    <div className="flex items-center gap-space8">
                        <FallBackImage src={data?.image_src ?? ''} fallback={data?.name.charAt(0) ?? ''} />
                        <article>
                            <Text title={data?.name} className='!text-lg font-medium' />
                            <Text variant='muted'>
                                {activeTab} | {data?.mobile} {data?.email && `| ${data.email}`}
                            </Text>
                        </article>
                    </div>
                    ...
                    <Button
                        ...
                        variant={'outline'}
                        onClick={() => {
                            setParty(data as IUserResponse);

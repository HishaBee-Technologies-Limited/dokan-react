...
const UserList = () => {
    ...
    return (
        <div className={`h-full w-full sm:max-w-[24.4rem] md:max-w-full lg:max-w-[24.4rem] flex-col gap-space12 ${activeUserItem ? 'hidden sm:flex md:hidden lg:flex' : 'flex'}`}>
            <Search onChange={() => { }} />
                    ...
                    {Array(40).fill(0).map((_, index) => (
                        ...
                        >
                            <div className="w-full flex items-center gap-space8">
                                <FallBackImage src='' fallback='M' className="h-space32 w-space32" />

                                <article>
                                    <Text title='নিজাম উদ্দিন' className='text-sm font-medium' />
                                    <Text title='01542141414' variant='muted' className='text-xs font-medium' />
                                </article>
                            </div>
                        </CardWithSideIndicator>
                    ))}

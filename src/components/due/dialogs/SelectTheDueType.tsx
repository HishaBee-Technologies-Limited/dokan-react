    ...
    return (
        <div>
            <div className="py-space16 space-y-space16 px-space16">
                ...
                <div className={`grid ${showCashType ? 'grid-rows-[1fr]' : 'grid-rows-[0fr]'} duration-300`}>
            ...
            <DialogFooter>
                {showCashType ? (
                    <Link href={activeCash === 'given' ? '/sell' : activeCash === 'received' ? '/purchase' : '#'} onClick={() => handleDialogClose({ open: false })}>
                        <Button className='w-full'>Continue</Button>
                    </Link>
                ) : (
                    <Button disabled={!showCashType} className='w-full'>Continue</Button>
                )}
            </DialogFooter>
        </div>
    )

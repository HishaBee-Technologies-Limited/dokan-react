...
const AsideBar = ({ menuOpen, setMenuOpen }: IMenuOpenProps) => {
    ...
    return (
        <aside className={`bg-white dark:bg-primary-90 h-screen fixed hd:relative ${menuOpen ? 'w-[30rem] xl:w-[34.4rem] left-0' : '-left-[30rem] xl:left-0 w-[30rem] xl:w-[8rem] hd:w-[34.4rem]'} duration-500 z-50`}>
                    ...
                    {SidebarLinks.map((menu) => {
                        ...
                        const menuLinkItem = (menu: IAsideBarMenuItem, subMenu: boolean) => (
                            <Link
                                ...
                                onClick={() => {
                                    ...
                                    setActive(subMenu ? active : active === menu.id ? null : menu.id)
                                }}
                                ...
                                <span className={`block text-text500 font-medium dark:text-text300 transition-all duration-700 ${menuOpen ? 'ml-space10' : 'w-0 overflow-hidden hd:h-auto hd:w-auto hd:ml-space10'}`}>{menu.title}</span>

                                {menu.children && (
                                    <div className={`absolute right-space12 top-1/2 transform -translate-y-1/2 ease-in ${active === menu.id ? 'rotate-180 duration-300' : 'duration-300'} ${menuOpen ? 'opacity-100 duration-1000' : 'xl:opacity-0 w-0 hd:opacity-100 hd:w-auto'}  `}>
                        ...
                        return (
                            <li key={menu.id}>
                                ...
                                {menu.children && (
                                    <div className={` ${menuOpen ? 'grid' : 'grid xl:hidden hd:grid'} ${active === menu.id ? 'grid-rows-[1fr] py-space10' : 'grid-rows-[0fr]'} duration-300 `}>

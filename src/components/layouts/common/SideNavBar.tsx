    ...
    return (
        <div className="h-full w-3/12 hidden md:block border-r border-color pr-space24">
            <ul className='space-y-space4 w-full'>
                {data.map((link) => (
                    <li key={link.title} className={`w-full rounded-md hover:bg-primary-10 dark:hover:bg-primary-90 ${pathname.includes(link.link) && pathname === link.link ? 'bg-primary-10 dark:bg-primary-90' : ''}`}>

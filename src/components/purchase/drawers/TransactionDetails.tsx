...
import { DeleteIcon, EditIcon, ExpandMoreIcon } from '@/components/common/icons'
...
const TransactionDetails = () => {
    ...
    return (
        <div className='space-y-space12'>
            <section className="bg-secondary rounded-lg p-space12 space-y-space16 ">
                ...
                <div className="flex items-center gap-space8">
                    ...
                    <div className="max-w-max py-space6 pl-space6 pr-space8 rounded-full flex items-center bg-white dark:bg-primary-90 border border-color">
                        <FallBackImage src='' fallback='M' className='w-space24 h-space24 mr-space8' />
                ...
                <Text title={`Transaction date  2/3/2023  |  12:30pm `} className='font-semibold' />
            </section>
            ...
            <article className="bg-secondary rounded-lg p-space12 space-y-space16 ">
                <article className="flex justify-between items-center gap-space8 border-b border-color pb-space12">
                    ...
                    <article className="flex gap-space8">
                        <Text title='Due' className='text-sm !text-white uppercase px-space8 py-space4 rounded-md bg-error-100' />
                        <Text title='Cash' className='text-sm !text-white uppercase px-space8 py-space4 rounded-md bg-blue-500' />
                    </article>
                </article>
            ...
            <section>
                <div onClick={() => setAccordion(!accordion)} className="flex justify-between items-center gap-space8 py-space8">
                    <Text title='Sold Product' className='text-lg font-medium' />

                    <ExpandMoreIcon />
                </div>

                <div className={`grid ${accordion ? 'grid-rows-[1fr]' : 'grid-rows-[0fr]'} duration-500`}>
            ...
            <article className='space-y-space8'>
                ...
                <Text title='This is a sample note for sample things' variant='secondary' className='text-lg' />
            </article>
            ...
            <div className="grid grid-cols-2 gap-space16">
                ...
                <Button className="w-full h-[9.6rem] flex-col" variant='secondary'>
                    <Image src="/images/share_receipt.svg" alt="" height={36} width={36} />
            ...
            <DrawerFooter>
                <Button
                    className="w-full"
                    variant={'danger'}
                    onClick={() => handleDialogOpen({ open: true, header: SellEnum.TRANSACTION_DELETE })}
                >
                    <DeleteIcon color='#fff' /> Delete
                </Button>
                <Button
                    className="w-full"
                    onClick={() => handleDrawerOpen({ open: true, header: SellEnum.TRANSACTION_EDIT })}
                >
                    <EditIcon /> Edit
                </Button>
            </DrawerFooter>

        ...
        <div className='space-y-space16'>
            ...
            <div className="grid gap-space16 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-2 2xl:grid-cols-3">
                {shopData.map((shop) => (
                    <Card
                        key={shop.id + 'shop'}
                        onClick={() => setSelectShop((prv) => shop.active ? prv : shop.id)}
                        className={`p-space16 w-full flex flex-col items-center relative shadow-sm border-color
                        ${shop.active ? '!bg-primary-5 dark:!bg-transparent border-[.2rem]' : 'cursor-pointer'}
                    ...
                    >
                        ...
                        <div className="mt-space32 flex flex-col items-center gap-space12">
                            ...
                            <Text title={shop.address} variant='secondary' className='text-sm' />
                ...
                <Card className={`p-space16 py-space40 w-full flex flex-col items-center shadow-sm border border-color border-color gap-space32`}>
                    <Image src={'/images/add_shop.svg'} alt='' height={84} width={84} />

                    <Link href={'/shop/add'} >
                        <Button variant='secondary' className='sm:px-space32'>
                            <Icon icon="fontisto:shopping-basket-add" height={24} width={24} />
            ...
            <div className="flex justify-end gap-space12 mt-space16">
                <Button disabled={!selectShop} onClick={handleContinue} >Continue</Button>
            </div>
        </div>
    )
}

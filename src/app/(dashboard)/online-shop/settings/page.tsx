...
const SettingPage = () => {
    ...
    return (
        <div className='w-full h-full flex flex-col gap-space16 '>
            <Tabs defaultValue={storeSettingOptions[0].value} onValueChange={(value) => setActiveTab(value)} >
                <TabsList className='flex gap-space8 flex-wrap justify-between'>
                    {storeSettingOptions.map((tab) => (
                        <TabsTrigger key={tab.value} value={tab.value} className="">{tab.label}</TabsTrigger>
                    ))}
                </TabsList>
            </Tabs>
